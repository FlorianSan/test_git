...
@version 1.11.0.0
@brief   \htmlonly
<span><img class="imgBriefImage" src="BloSwiNc32x32.png" alt="BloSwiNc32x32.png"></span>
<span class="spanBriefText">Block "Switch Normally Closed" (<b>BloSwiNc</b>)</span>
\endhtmlonly

@details
The block "Switch NC" should be used to connect a switch of "Normally Closed" Type to a controller.
...
Read more - \ref TSwiNcPrp::eAdcType

\htmlonly <div class=divMaxWidth style="max-width: 910px;">
  <img class="imgInCenter" width="600px" align="left" src="BloSwiNc_Input.png" alt="BloSwiNc_Input.png">
</div> <table width="100%"></table> \endhtmlonly

Also \ref TSwiNcOut::u16RawVal can be used as input from a CAN-bus message,
if pin index \ref TSwiNcPrp::ePin is set to "PIN_MAX".

### Output parameters
 - tOut.bi2SwiNcState - The state of a switch.
 - tOut.u16RawVal - The measured value at the pin of a switch.

\htmlonly <div class=divMaxWidth style="max-width: 910px;">
  <img class="imgInCenter" width="600px" align="left" src="BloSwiNc_Output.png" alt="BloSwiNc_Output.png">
</div> <table width="100%"></table> \endhtmlonly

If a switch is not operated ( closed position ) then the state is "OFF". \n
If a switch is operated ( open position ) position, the output status is set to ON.\n
Type of switch connection:
 - \ref DM_SWI_NC_HIGH_SIDE - external supply of a switch
 - \ref DM_SWI_NC_LOW_SIDE - internal supply of a switch
...
| \ref TSwiNcPrp::ePin "ePin" - pin index                         | enum.     |                   | PIN_NA              |
| \ref TSwiNcPrp::eAdcType "eAdcType" - Type of pin               | enum.     |                   | ADC_VIN_V           |
| \ref TSwiNcPrp::eSwiNcType "eSwiNcType" - Type of switch connection | enum. |                   | DM_SWI_NC_HIGH_SIDE |
| \b Parameters                                                   |           |                   |                     |
| \ref TSwiNcPar::bi2SwiDflt "bi2SwiDflt" - Default input value                  | [def] | BI2_OFF ... BI2_ON | BI2_OFF |
| \ref TSwiNcPar::u32DebTimeSwitches "u32DebTimeSwitches" - Debounce time        | [ms]  | 0 ... U32_LIMIT-1  | 100     |
| \ref TSwiNcPar::u16VolLoMin "u16VolLoMin" - Low level signal :: Minimal limit  | [def] | 0 ... U16_LIMIT-1  | 0       |
| \ref TSwiNcPar::u16VolLoMax "u16VolLoMax" - Low level signal :: Maximal limit  | [def] | 0 ... U16_LIMIT-1  | 2000    |
| \ref TSwiNcPar::u16VolHiMin "u16VolHiMin" - High level signal :: Minimal limit | [def] | 0 ... U16_LIMIT-1  | 8000    |
| \ref TSwiNcPar::u16VolHiMax "u16VolHiMax" - High level signal :: Maximal limit | [def] | 0 ... U16_LIMIT-1  | 29000   |
...
-   1.5.1.0 20.07.2017  
    - The block output value is debounced at erroneous input if input behavior 
      at error is set to INP_BEH_PAR_TO_INP.
-   1.6.0.0 26.06.2018  
...
//******************************************************************************************************
...
#ifndef __ITFBLOSWINC__
    #define __ITFBLOSWINC__
    ...
    enum
    {
        DM_SWI_NC_PIN_LOW,              //!< 0 - An input signal is too low / short circuit to ground
        /*!<
         The input value is lower than minimal limit of low level signal.
         */
        DM_SWI_NC_PIN_HIGH,             //!< 1 - An input signal is too high / short circuit to power
        /*!<
         The input value is higher than maximal limit of high level signal.
         */
        DM_SWI_NC_RANGE,                //!< 2 - An input signal is out of valid range
        /*!<
         ...
         */
        DM_SWI_NC_LIMIT_PARAMETER,      //!< 3 - Warning: a block has limited parameters
        /*!<
         ...
         */
        DM_SWI_NC_INIT_ERROR,           //!< 4 - An initialization error
        /*!<
         ...
         */
        DM_SWI_NC_CNT_MAX               //! Quantity of error detection methods
    };

    //! Configuration
    typedef enum
    {
        DM_SWI_NC_HIGH_SIDE,            //!< 0 - Switch normally closed is connected to high-side
        /*!<
        ...
        @htmlonly <div class="divMaxWidth">
          <img width=480px align="center" src="BloSwiNc_PullDown.png" alt="BloSwiNc_PullDown.png">
        </div> <table width="100%"></table> @endhtmlonly
         */
        DM_SWI_NC_LOW_SIDE,             //!< 1 - Switch normally closed is connected to low-side
        /*!<
        If a switch is connected to the low-side ( ground ) then pin resistor have to be
        set in the "pull-up" position.
        @htmlonly <div class="divMaxWidth">
          <img width=480px align="center" src="BloSwiNc_PullUp.png" alt="BloSwiNc_PullUp.png">
        </div> <table width="100%"></table> @endhtmlonly
         */
        DM_SWI_NC_TYP_UNDEF             //!< 2 - Undefined type ( not a valid value )
    }ESwiNcTyp;
    ...
    typedef struct
    {
        ...
        @htmlonly <div class="divMaxWidth">
          <img width=700px align="center" src="BloSwiNc_Input.png" alt="BloSwiNc_Input.png">
        </div> <table width="100%"></table> @endhtmlonly

        #### ADC_DIN_D - Digital input
        If the pin type is configured as the digital input then limits of high and low levels are
        irrelevant. \n
        Also this type of pin does not recognize the following errors: \ref DM_SWI_NC_PIN_LOW,
        \ref DM_SWI_NC_PIN_HIGH, \ref DM_SWI_NC_RANGE
         */
        ESwiNcTyp   eSwiNcType;         //!< [enu] Switch connection type ( High-side / Low-side )
        /*!<
        #### High-Side
        If a switch is connected to the high-side ( power supply ) then pin resistor have to be
        set in the "pull-down" position.
...
#ifdef BLOCK_DB_CONNECTION_SWINC
    ...
    typedef struct
    {
        ...
        // Properties
        TSwiNcPrp    tPrp;                              //!< [stu] - Properties

        // Parameters
        TSwiNcPar    tPar;                              //!< [stu] - Parameters
        
        // Errors
        TUint8     u8ErrCnt;                            //!< [num] - Quantity of defined errors
        TErrCfgFea atErrFea[DM_SWI_NC_CNT_MAX];         //!< [cla] - Error Configurations
    }TSwiNcCfg;
    ...
    typedef struct
    {
        ...
        TBit2       bi2SwiNcState;      //!< [def] Output state
    ...
    typedef struct
    {
        const TSwiNcCfg* cptCfg;        //!< [stu] Configuration structure of the block
    ...
    typedef struct
    {
        TSwiNcInp   tInp;               //!< [stu] Input vector
        TSwiNcOut   tOut;               //!< [stu] Output vector
        TSwiNcPrp   tPrp;               //!< [stu] Properties
        TSwiNcPar   tPar;               //!< [stu] Parameters
        TSwiNcAdr   tXAdr;              //!< [stu] Address information
    }TBloSwiNc;
    ...
    //--------------------------------------------------------------------------------------------------
    extern const TVerChapCom *cptBloSwiNcVersionsInfo( TVoid );
    ...
    //--------------------------------------------------------------------------------------------------
    extern TBoolean boBloSwiNcVersionCheck( TVoid );
    ...
    //!             - Main structure \ref TBloSwiNc must be already defined
    //!             - Configuration structure \ref TSwiNcCfg must be already defined
    //! @note
    ...
    //! @retval     R_NOT_INITIALIZED - Pin is not selected ( equal to PIN_NA )
    //--------------------------------------------------------------------------------------------------

    extern ERetVal eBloSwiNcCreate( TBloSwiNc* ptBlo, const TSwiNcCfg* cptCfg );
    ...
    //! @pre
    //!             - Function \ref eBloSwiNcCreate must be already successfully executed.
    //! @note
    //!             - Configuration structure, which was set in \ref eBloSwiNcCreate "creation
    ...
    //! @retval     R_NOT_REGISTRY      - Object is not registered
    //! @retval     R_PARAMETER         - Same as error \ref DM_SWI_NC_INIT_ERROR
    //--------------------------------------------------------------------------------------------------
    extern ERetVal eBloSwiNcInit(TBloSwiNc* ptBlo );
    ...
    //! @retval     R_ADDRESS       - Wrong address of object
    //--------------------------------------------------------------------------------------------------
    extern ERetVal eBloSwiNcCreateInitRegistry( TVoid *pvBlo );
    ...
    //!             or without it.
    //! @pre
    //!             - Object must be defined in PDT or function \ref eBloSwiNcInit must be already
    ...
    //! @retval     R_ADDRESS       - Wrong address of object
    //--------------------------------------------------------------------------------------------------
    extern ERetVal eBloSwiNc( TBloSwiNc* ptBlo );
    ...
    //--------------------------------------------------------------------------------------------------
    extern TBoolean boBloSwiNcGetErrStaBit( TBloSwiNc* ptBlo, TUint8 u8BitPos );
    ...
    //--------------------------------------------------------------------------------------------------
    extern TUint16 u16BloSwiNcGetErrStaAll( TBloSwiNc* ptBlo );
    ...
    //! @retval     FALSE       - Wrong/null pointer to the block object
    //--------------------------------------------------------------------------------------------------
    extern TBoolean boBloSwiNcGetErrEveBit( TBloSwiNc* ptBlo, TBoolean boDetect, TUint8 u8BitPos );
    ...
    //--------------------------------------------------------------------------------------------------
    extern TUint16 u16BloSwiNcGetErrEveAll( TBloSwiNc* ptBlo, TBoolean boDetect );
