...
//-----------------------------------------------------------------------------------------------------
//! @brief control one hydraulic proportional function 2
//!
//! @param[in] 	i16JoystickScaling 				- joystick scaling
//! @param[in] 	boAutorizeUp 					- autorize to use up function (joystick scaling > 0)
//! @param[in] 	boAutorizeDown 					- autorize to use down function (joystick scaling < 0)
...
//! -# 17.12.2018   CK: Add output supply pvg
//! -# 07.01.2019   CK: Add 2 more command range , to apply consigne min up mouvement and down
//! -# 16.01.2019   CK: change signal RampRS2 to RampTS4
//! -# 28.01.2019   CK: Add output parameter to get setpoint after ramp joystick
...
TInt16 i16ProportionalFunction2(TInt16 i16JoystickScaling, TBoolean boAutorizeUp,TBoolean boAutorizeDown,
							   TSigRampAdr* ptRampSignal, TUint16 auRampRatio[4], TInt16 aiCommandRange[5],TBoolean *boSupplyPvg,TInt16 *i16SetpointAfterRampJoystick)
{
	/* local variable declaration*/
	TInt16 		i16Setpoint 			= 0;
	TInt16 		i16SetpointAfterRamp;
	...
	/* set setpoint as input of the ramp and run the function ramp*/
	i16SetpointAfterRamp = i16SigRampTS4(ptRampSignal, i16Setpoint, u16RampTime[0], u16RampTime[1], u16RampTime[2], u16RampTime[3]);
	...
	/* return value of joystick after ramp*/
...
//! @param[in] 	boAutorizeUp 			- autorize to use up function (joystick scaling > 0)
//! @param[in] 	ptRampSignal 			- ramp signal function
//! @param[in] 	auRampRatio[2] 			- ramp ratio increase / decrease apply to joystick
//! @param[in] 	aiCommandRange[3] 	- Command off engine / min / max
//! @param[out]	i16Command			- Command is return by the function
//!
//! @globvar
//!
//! @staticvar
...
//! -# 04.09.2018   CK: Adapt fonction for 1 prop input to pilot 1 prop output without ramp
//! -# 11.01.2019   CK: Add ramp
//! @care
//! @todo
//----------------------------------------------------------------------------------------------------
TInt16 In1ProAxisPilotOut1PropEvFunction(TInt16 i16JoystickScaling, TBoolean boAutorizeUp,TSigRampAdr* ptRampSignal, TUint16 auRampRatio[2],TInt16 aiCommandRange[3])
{
	/* local variable declaration*/
	TInt16 		i16Setpoint = 0;
	TInt16 		i16SetpointAfterRamp;
	...
	if (i16SetpointAfterRamp > 0)
	{
		i16Command = i16CalcLinear2Dots(i16Setpoint, 0, (aiCommandRange[1]),
				10000, (aiCommandRange[2]));
	}
	else
	{
...
//! @todo
//----------------------------------------------------------------------------------------------------
TVoid In1ProAxisPilotOut2PropEvFunction(TInt16 i16JoystickScaling,TBoolean boJoystickCentred,/*//NEOTEC WARNING TUint16 u16JoystickBrut,*/ TBoolean boAutorizeUp,TBoolean boAutorizeDown,
...
//-----------------------------------------------------------------------------------------------------
//! @brief Return an angle between - max angle and + max angle (multi turn possible)
//!
//! @param[in] 		u32Angle 				- Currente angle read by the sensor
//! @param[in] 		u32CenterAngle 			- this angle correspond to the center, 0°
//! @param[in] 		u16Resolution 			- resolution for one turn
//! @param[out]   	*u16Command				- Command
//!
//!
//! @globvar
...
//! @todo
//-----------------------------------------------------------------------------------------------------
TInt16 i16GetTurretAngle(TUint32 u32Angle, TUint32 u32CenterAngle, TUint16 u16Resolution)
{
	/* local variable*/
	TInt16 i16Angle;
	TInt16 i32Gap;

	/* get gap between current angle and center
	 if center is lower than 1 complete turn*/
	if (u32CenterAngle - u16Resolution > u32CenterAngle)
	{
		if ((u32Angle >  u32CenterAngle) && (u32Angle > u32CenterAngle + u16Resolution))
		{
			i32Gap = u32Angle - U32_MAX - u32CenterAngle;
		}
		else
		{
			i32Gap = u32Angle - u32CenterAngle;
		}
	}
	/* if center is lower than 1 complete turn*/
	else if (u32CenterAngle + u16Resolution < u32CenterAngle)
	{
		if ((u32Angle <  u32CenterAngle) && (u32Angle < u16Resolution))
		{
			i32Gap = U32_MAX - u32CenterAngle + u32Angle;
		}
		else
		{
			i32Gap = u32Angle - u32CenterAngle;
	...
	/*calculate angle*/
	i16Angle = i32CalcLinear2Dots(i32Gap, -u16Resolution, 3600, u16Resolution, -3600);
