...
TVoid vDiagSystem(TVoid)
{
	...
	gDb_tRamGeneralC1.eWatchdogState = (EWdState) tDiag.eDiagState;

	/* Increment LifeByte variable every cycle to know if the ECU is blocked or not*/
	if (gDb_tRamGeneralC1.u8LifeByte == U8_MAX){gDb_tRamGeneralC1.u8LifeByte = 0;}
		gDb_tRamGeneralC1.u8LifeByte++;

	/* get the average cycle time of this application*/
	gDb_tRamGeneralC1.u32AverageTimeCycle = g_tStatistic.u32Average;
	/* get the max cycle time of this application, It must be less than the time defined for the application*/
	gDb_tRamGeneralC1.u32MaxTimeCycle = g_tStatistic.u32MaxTime;

	/* read batteries voltage ECU*/
	gDb_tRamGeneralC1.u16EcuVoltage = u16CoreAdcGetPin(ADC_SYS_BAT_CURRENT,PIN_SYS);

//NEOTEC IG DEBUG
	gDb_tRamGeneralC1.u16FaultyValue = tDiag.tDiagError.u16FaultyValue;
	gDb_tRamGeneralC1.u8DeviceNum = tDiag.tDiagError.u8DeviceNum;
	gDb_tRamGeneralC1.u8ErrorCode = tDiag.tDiagError.u8ErrorCode;
//NEOTEC IG DEBUG
	/* ============================================================================================= */
	/* Get error	*/
	/* ============================================================================================= */
	...
	TInt16  i16i =0;
		...
		/*get the error stack information for all active and inactive errors (ERR_FILTER_ACTIVE)*/
		u8ErrListCntMax = u8CoreErrFaultFilterStackCopy(ACC_DEVELOPMENT,ERR_FILTER_ACTIV,terror,tErrorExt);
		gDb_tRamGeneralNeotecC1.u8RamGeneralNeotecC1_2 = u8ErrListCntMax;

		if(u8ErrListCntMax == 0)
		{
			gDb_tRamGeneralC1.boActiveFailure = FALSE;
		}
		else
		{
			gDb_tRamGeneralC1.boActiveFailure = TRUE;
		}
